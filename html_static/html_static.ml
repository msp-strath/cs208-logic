...
struct
  let escape_attribute str =
    let rec loop i =
      ...
      | None -> Output.substring str i (String.length str - i)
      | Some j when j = i ->
          Output.string "&quot;";
          loop (i + 1)
      | Some j ->
          Output.substring str i (j - i);
          Output.string "&quot;";
          loop (j + 1)
    in
    loop 0
  ...
  let attributes attrs =
    if not (AttributeMap.is_empty attrs) then
      AttributeMap.iter
        (fun name value ->
          Output.string " ";
          Output.string name;
  ...
  let escaped_text text =
    let rec loop i j =
      if j = String.length text then Output.substring text i (j - i)
      else if text.[j] = '<' then (
  ...
  let rec node = function
    | Normal_element (name, attrs, children) ->
        ...
        node children;
        end_tag name
    | Void_element (name, attrs) -> start_tag name attrs
    ...
    | EscapableRawText_element (name, attrs, text) ->
        ...
        Output.string text;
        end_tag name
    | Text string -> escaped_text string
    | RawText string -> Output.string string
    | Concat (node1, node2) ->
        node node1;
        node node2
    | Empty -> ()
    | List nodes -> List.iter node nodes

  let document ?(doctype = true) document =
    if doctype then Output.string "<!DOCTYPE html>";
    node document
end

module Render = struct
  let to_custom ?doctype render document =
    let module R = RenderCommon (struct
      let string = render
      let substring s i len = render (String.sub s i len)
    end) in
  ...
  let to_channel ?doctype ch document =
    let module Renderer = RenderCommon (struct
...
(**********************************************************************)
type _ attribute = Attr of string * string | NoAttr
...
let map _f t = t
let of_seq seq = Seq.fold_left ( ^^ ) empty seq

let attributes list =
  List.fold_left
    (fun attributes attr ->
      match attr with
      | Attr (name, value) -> AttributeMap.add name value attributes
      | NoAttr -> attributes)
    AttributeMap.empty list

let raw_attr name value = Attr (name, value)
...
let raw_text text = RawText text
...
module A = struct
  ...
  let hidden value = Attr ("hidden", if value then "true" else "false")
  let id value = Attr ("id", value)
  let lang value = Attr ("lang", value)
  let spellcheck value = Attr ("spellcheck", if value then "true" else "false")
  let style value = Attr ("style", value)
  let tabindex value = Attr ("tabindex", string_of_int value)
  let title value = Attr ("title", value)
  let translate value = Attr ("translate", if value then "yes" else "no")

  (* 'html' element attributes (4.1.1) *)
  let manifest value = Attr ("manifest", value)

  (* 'meta' attributes *)
  let http_equiv value = Attr ("http-equiv", value)
  let content value = Attr ("content", value)
  let charset value = Attr ("charset", value)

  let crossorigin = function
    | `anonymous -> Attr ("crossorigin", "anonymous")
    | `use_credentials -> Attr ("crossorigin", "use-credentials")
  ...
  let label value = Attr ("label", value)
  let placeholder value = Attr ("placeholder", value)
  let value value = Attr ("value", value)
  let enabled value = Attr ("enabled", if value then "yes" else "no")
  let checked value = if value then Attr ("checked", "yes") else NoAttr
  let selected value = Attr ("selected", if value then "yes" else "no")
  let disabled value = if value then Attr ("disabled", "yes") else NoAttr

  (* For 'label' elements *)
  let form value = Attr ("form", value)
  let for_control value = Attr ("for", value)
