...
\begin{frame}
  ...
  \item When a formula is true in some model, we write:
    \begin{displaymath}
      \mathcal{M} \models P
    \end{displaymath}
  \item Our proof system is sound and (with excluded middle) complete for this semantics
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Fix formulas $P_1, ..., P_n$. Is there a model for them? \\
    \textcolor{black!60}{Generating ``worlds''}
...
\begin{frame}
  ...
  \begin{displaymath}
    \begin{array}{cl}
      ...
      = & \false \qquad \textrm{\textcolor{black!60}{(because no such $a$ exists)}}
    \end{array}
  \end{displaymath}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Proposed by Edward F. Codd in 1970.
