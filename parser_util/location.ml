type t =
  | Generated
  | FromSource of { loc_start : Lexing.position; loc_end : Lexing.position }

let mk loc_start loc_end = FromSource { loc_start; loc_end }
...
let pp_without_filename fmt = function
  ...
  | FromSource { loc_start; loc_end } when loc_start.pos_lnum = loc_end.pos_lnum
    ->
      Format.fprintf fmt "line %d, characters %d-%d" loc_start.pos_lnum
        (loc_start.pos_cnum - loc_start.pos_bol)
        (loc_end.pos_cnum - loc_end.pos_bol)
  | FromSource { loc_start; loc_end } ->
